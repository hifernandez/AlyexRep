...
$p_cabezote_prensa =$_POST['p_txtTempDado'];
...
if ($stmt = $mysqli->prepare("CALL sp_insertKardexTRSAS(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"))
{
    //Vinculando parametros a la consultad
    if (!$stmt->bind_param("iiisdsddddddddddsis",$p_comboClaveKardex,    $p_comboClaveTurno,    $p_comboClaveStatus,    $p_spinHoraInicio,    $p_spinTimeCalenta,    $p_dtFechaExtru,    $p_txtPresExtru,    $p_cabezote_prensa,    $p_txtTempBols,    $p_txtTempCont,    $p_txt3Lingotes,    $p_txtTempOper,    $p_txtTempExt,    $p_txtVelExtr,    $p_txtKGBrutos,    $p_txtLingExt,    $p_btnTipoExt,    $p_btnCantCuna,    $p_txtComentExtru            )) {
